...
Sweeping over 100 values of {math}`\gamma` on {math}`(0,2.61]` and 100
values of {math}`\delta` on {math}`[-1,1]` with {math}`L=1` gives the plot
below, where each blue dot denotes a parameter pair with a feasible certificate.
