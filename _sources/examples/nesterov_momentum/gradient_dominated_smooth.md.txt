...
{math}`\delta \in (-1,1)` with {math}`\mu_{\textup{gd}}=0.5` and
...
```{image} ../../_static/nesterov_momentum_gradient_dominated_smooth.svg
:alt: Constant Nesterov momentum feasible (gamma, delta) pairs for gradient-dominated smooth objectives, with color showing rho.
:align: center
:width: 100%
```
